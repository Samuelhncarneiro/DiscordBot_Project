...
def main():
    ...
    with Pool(args.threads, initializer=init) as p:
        ...
        train = list(tqdm(p.imap(
            annotate_, train, chunksize=args.batch_size), total=len(train), desc='train'))
        dev = list(tqdm(p.imap(annotate_, dev, chunksize=args.batch_size),
                   total=len(dev), desc='dev  '))
    ...
    vocab, counter = build_vocab([row[5] for row in full], [
                                 row[1] for row in full], wv_vocab, args.sort_all)
    ...
    for article in data:
        for paragraph in article['paragraphs']:
            ...
            for qa in paragraph['qas']:
                ...
                if mode == 'train':
                    # in training data there's only one answer
...
def annotate(row, wv_cased):
    ...
    question_lemma = {w.lemma_ if w.lemma_ !=
                      '-PRON-' else w.text.lower() for w in q_doc}
    question_tokens_set = set(question_tokens)
    question_tokens_lower_set = set(question_tokens_lower)
    match_origin = [w in question_tokens_set for w in context_tokens]
    match_lower = [
        w in question_tokens_lower_set for w in context_tokens_lower]
    ...
    context_features = list(
        zip(match_origin, match_lower, match_lemma, context_tf))
    if not wv_cased:
...
def build_vocab(questions, contexts, wv_vocab, sort_all=False):
    ...
    if sort_all:
        ...
        vocab = sorted([t for t in counter_q if t in wv_vocab],
                       key=counter_q.get, reverse=True)
